  ...
  await withServer(
    (req, res) => {
      if (req.url === '/chat/completions') {
        ...
        req.on('end', () => {
          res.write(
            'data: {"choices":[{"index":0,"delta":{"role":"assistant","content":null,"tool_calls":[{"index":0,"id":"call_1","type":"function","function":{"name":"read_file","arguments":""}}]},"finish_reason":null}]}\n\n',
          );
          res.write(
            'data: {"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\\"path\\":"}}]},"finish_reason":null}]}\n\n',
          );
          res.write(
            'data: {"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\\"test.txt\\"}"}}]},"finish_reason":"tool_calls"}]}\n\n',
          );
          res.write('data: [DONE]\n\n');
          res.end();
        });
  ...
  await withServer(
    (req, res) => {
      if (req.url === '/chat/completions') {
        ...
        res.end(
          JSON.stringify({ error: { message: 'Invalid authentication', code: 'invalid_api_key' } }),
        );
      }
    },
    async (baseURL) => {
