...
export async function buildSystemPrompt({
  ...
  const notionConnected = !!(config?.notion?.api_key && config?.notion?.gdd_page_id);
  ...
  const notionText = notionConnected
    ? 'Notion GDD terhubung. Kamu bisa baca/tulis Game Design Document dari Notion pakai tool read_gdd_notion dan write_gdd_notion.'
    : '(Notion GDD belum di-configure — set notion.api_key + notion.gdd_page_id di config untuk akses)';
  ...
  const toolsByMode = {
    execute: [
      ...
      .filter(Boolean)
      .join(', '),
    plan: ['read_file, list_dir', obsidianConnected ? 'obsidian_search, obsidian_read_note' : null]
      .filter(Boolean)
      .join(', '),
    general: [
      'read_file, write_file, edit_file, list_dir, run_command',
