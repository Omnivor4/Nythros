...
export const generateDocxTool = {
  name: "generate_docx",
  description: "Buat file dokumen Microsoft Word (.docx) dengan format teks dasar (heading & paragraf). Cocok untuk laporan tugas.",
  input_schema: {
    type: "object",
    properties: {
      ...
      content: { 
        ...
        items: {
          type: "object",
          properties: {
            type: { type: "string", enum: ["heading", "paragraph"], description: "Jenis blok teks" },
  ...
  execute: async ({ filename, title, content }) => {
    try {
      ...
      return `Error membuat DOCX: ${err.message}`;
    }
  },
};
...
export const generateXlsxTool = {
  name: "generate_xlsx",
  description: "Ekspor data ke format Excel (.xlsx). Cocok untuk balance tracker atau data tabular.",
  input_schema: {
    type: "object",
    properties: {
  ...
  execute: async ({ filename, sheetName = "Sheet1", columns, rows }) => {
    try {
      ...
      return `Error membuat XLSX: ${err.message}`;
    }
  },
};
...
export const generatePdfTool = {
  ...
  execute: async ({ filename, text }) => {
    try {
      ...
      return `Error membuat PDF: ${err.message}`;
    }
  },
};
