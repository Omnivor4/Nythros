...
export const readFileTool = {
  ...
  execute: ({ path: p }) => {
    ...
    if (stat.size > 2 * 1024 * 1024)
      return `Error: File terlalu besar (${stat.size} bytes). Maksimal 2MB.`;

    if (stat.isDirectory())
      return `Error: "${p}" adalah sebuah folder, bukan file. Gunakan list_dir untuk melihat isinya.`;
...
export const editFileTool = {
  name: 'edit_file',
  description:
    'Ganti satu potongan teks unik di dalam file (find & replace persis, bukan overwrite seluruh file).',
  input_schema: {
    type: 'object',
    properties: {
      path: { type: 'string' },
  ...
  execute: ({ path: p, old_text, new_text }) => {
    ...
    try {
      ...
      return `Error saat edit file: ${safeError(e)}`;
    }
  },
};
...
export const listDirTool = {
  ...
  execute: ({ path: p = '.' }) => {
    ...
    if (!fs.statSync(p).isDirectory())
      return `Error: "${p}" adalah file, bukan folder. Gunakan read_file untuk membacanya.`;
...
export const runCommandTool = {
  name: 'run_command',
  description:
    'Menjalankan perintah shell/terminal (misal: npm test, git grep, ls). Hati-hati dengan command destruktif.',
  input_schema: {
    type: 'object',
    properties: {
  ...
  execute: async ({ command }) => {
      ...
      if (looksDangerous(command)) {
        if (getConfirmMode() === 'ask') {
          const confirmed = await askConfirmation(
            `Command "${command}" looks dangerous and will run directly on host. Execute?`,
          );
        ...
        const confirmed = await askConfirmation(
          `⚠️ Sandbox warning: Docker not found. Command will run directly on host: "${command}". Execute?`,
        );
        if (!confirmed) return 'Command execution cancelled by user.';
      }
    }
    ...
    if (sandboxMode === 'docker') {
      ...
      if (userNetworkMode === 'auto') {
        if (needsNetwork(command)) {
          if (getConfirmMode() === 'ask') {
            network = await askConfirmation(
              `Command "${command}" kelihatannya butuh akses internet. Izinkan container ini akses network untuk kali ini?`,
            );
          }
        }
      } else {
      ...
      try {
        const out = await runInDocker(command, {
    ...
    return new Promise((resolve) => {
      ...
      exec(command, { maxBuffer: 1024 * 1024 * 2, timeout: 60000 }, (error, stdout, stderr) => {
        let out = '';
...
export const queryArchiveTool = {
  name: 'query_archive',
  description:
    'Cari atau baca ringkasan percakapan sebelumnya yang sudah diarsipkan. Gunakan ini kalau butuh konteks dari sesi atau percakapan yang sudah lama (tidak ada di working memory saat ini). Input: keyword untuk search, atau kosong untuk baca 5 ringkasan terbaru.',
  input_schema: {
    type: 'object',
    properties: {
      keyword: {
