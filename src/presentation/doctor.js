...
function checkSystem() {
  ...
  checks.push({ status: "ok", msg: `Node.js ${process.version} on ${process.platform} (${os.arch()})` });
  checks.push({ status: "ok", msg: `${os.cpus().length} CPU core, ${memGB}GB RAM (${freeMemGB}GB free, ${freePercent}%)` });
...
function checkHomeDir() {
  ...
  if (fs.existsSync(HOME_DIR)) {
    ...
    checks.push({ status: "err", msg: `Home dir ${HOME_DIR} tidak ditemukan. Jalankan 'nythros setup' atau 'nythros doctor --fix'.` });
    return checks;
  }
...
function checkConfig(config) {
  ...
  config.endpoints.forEach((ep, i) => {
    ...
    if (issues.length === 0) {
      checks.push({ status: "ok", msg: `${name}: ${ep.base_url} → ${ep.model || "(model tidak diset)"}` });
    } else {
...
async function verifyEndpoint(ep) {
  try {
    ...
    if (res.ok) {
      ...
      return { status: "ok", msg: `${ep.base_url} — ${res.status} OK, ${modelCount} model terdaftar`, modelList: modelList.slice(0, 50) };
    }
    ...
    if (err.cause?.code === "ECONNREFUSED") {
      return { status: "err", msg: `${ep.base_url} — koneksi ditolak (ECONNREFUSED). Server nggak jalan?` };
    }
...
function checkProjectDir() {
  ...
  if (PROJECT_DIR && PROJECT_DIR !== HOME_DIR) {
    ...
    if (fs.existsSync(archivePath)) {
      ...
      checks.push({ status: "ok", msg: `Archive: ${lines} entri (${(stats.size / 1024).toFixed(1)} KB)` });
    } else {
...
function getSuggestions(allChecks) {
  ...
  if (!configuredEndpoint && pendingEndpoint) {
    suggestions.push("Endpoint belum lengkap — jalankan 'nythros setup' untuk isi base_url, api_key, dan model.");
  } else if (!configuredEndpoint) {
    suggestions.push("Belum ada endpoint — jalankan 'nythros setup' untuk konfigurasi endpoint AI.");
  }
...
export { checkSystem, checkHomeDir, checkConfig, verifyEndpoint, checkProjectDir, checkObservations, getSuggestions, collectAllChecks };
...
export async function runDoctor(verbose = false) {
  ...
  const sysChecks = checkSystem();
  sysChecks.forEach(c => { console.log(indent(`${icon(c.status)} ${c.msg}`)); allChecks.push({ ...c, section: "system" }); });
  if (verbose) {
    console.log(indent(`${C.dim}  Uptime: ${Math.round(os.uptime() / 3600)} jam, User: ${os.userInfo().username}${C.reset}`));
  }
  console.log();
  ...
  const homeChecks = checkHomeDir();
  homeChecks.forEach(c => { console.log(indent(`${icon(c.status)} ${c.msg}`)); allChecks.push({ ...c, section: "home" }); });
  try {
    config = loadConfig();
    const cfgChecks = checkConfig(config);
    cfgChecks.forEach(c => { console.log(indent(`${icon(c.status)} ${c.msg}`)); allChecks.push({ ...c, section: "config" }); });
  } catch (err) {
  ...
  if (verbose && config) {
    ...
    if (ep) {
      console.log(indent(`${C.dim}  ${JSON.stringify({ id: ep.id, base_url: ep.base_url, model: ep.model, api_key: ep.api_key ? '***' : '' }, null, 4)}${C.reset}`));
    }
  }
  console.log();
  ...
  const projChecks = checkProjectDir();
  projChecks.forEach(c => { console.log(indent(`${icon(c.status)} ${c.msg}`)); allChecks.push({ ...c, section: "project" }); });
  const obsChecks = checkObservations();
  obsChecks.forEach(c => { console.log(indent(`${icon(c.status)} ${c.msg}`)); allChecks.push({ ...c, section: "observations" }); });
  console.log();
    ...
    if (verifiable.length === 0) {
      ...
      for (const ep of verifiable) {
        ...
        if (verbose && result.status === "ok" && result.modelList?.length > 0) {
          const names = result.modelList.map(m => m.id).slice(0, 15).join(", ");
          console.log(indent(`  ${C.dim}Models: ${names}${result.modelList.length > 15 ? `, ... +${result.modelList.length - 15}` : ""}${C.reset}`));
        }
        allChecks.push({ ...result, section: "verify" });
        verifyResults.push(result);
      }
  ...
  if (err === 0 && warn === 0) {
    ...
    console.log(`${C.red}${C.bold}❌ ${total} cek: ${ok} OK, ${warn} peringatan, ${err} error.${C.reset}\n`);
  }
}
...
export async function doctorFix() {
  ...
  if (homeErr) {
    ...
    try {
      ...
      allChecks.push({ status: "ok", section: "home", msg: "Struktur direktori terbuat (memory, cache, logs, dll)" });
    } catch (e) {
      console.log(indent(`${icon('err')} Gagal buat direktori: ${e.message}`));
      allChecks.push({ status: "err", section: "home", msg: `Gagal buat ${HOME_DIR}: ${e.message}` });
    }
  } else {
    homeChecks.forEach(c => allChecks.push({ ...c, section: "home" }));
  }
  ...
  const needEndpoint = !config?.endpoints?.length ||
    config.endpoints.some(ep => !ep.base_url || !ep.api_key);
  ...
  if (needEndpoint && process.stdout.isTTY) {
    ...
    const baseURL = await askQuestion(indent(`Base URL [${ep.base_url || 'https://openrouter.ai/api/v1'}]: `));
    const apiKey = await askQuestion(indent(`API Key [${ep.api_key ? '***sudah terisi***' : 'kosong'}]: `));
    ...
    console.log(indent(`${icon("warn")} Non-interactive mode — skip fix. Jalankan 'nythros setup' manual.`));
  }
  ...
  if (err === 0 && warn === 0) {
    ...
    console.log(`${C.yellow}${C.bold}⚠️  ${total} cek: ${ok} OK, ${warn} peringatan + perbaikan selesai.${C.reset}\n`);
  } else {
    console.log(`${C.red}${C.bold}❌ ${total} cek: ${ok} OK, ${warn} peringatan, ${err} error.${C.reset}\n`);
  }
}
