 ...
 */
export async function runInDocker(command, {
  ...
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      "--name", containerName,
      "--network", networkArg,
      "-v", `${path.resolve(projectRoot)}:${workspacePath}`,
      "-w", workspacePath,
      "--cpus", "1",
      "--memory", "512m",
      "--pids-limit", "256",
    ...
    proc.on("error", (err) => {
      ...
      if (err.code === 'ENOENT') {
        reject(new Error("Docker daemon tidak ditemukan (docker command missing). Pastikan Docker terinstall dan berjalan."));
      } else {
